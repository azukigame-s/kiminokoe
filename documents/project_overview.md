...
- **サイズ**: 2px
- **色**: 黒（不透明度90%）
- **適用対象**: ゲーム画面の本文テキスト（RichTextLabel）
...
- **実装予定**: 下部メニュー実装時にスキップインジケータ（右上）を廃止して統合

#### ポーズメニュー

- **開閉**: Escapeキー（下部メニューの「メニュー」ボタンでも開く）
- **実装**: `scripts/ui/pause_menu.gd`
- **ボタン**: ゲームに戻る / バックログ / セーブ / ロード / トロフィー / 設定 / タイトルへ戻る
- **状態**: バックログ・セーブ・ロード・トロフィーは未実装（disabled）
