...
Webサイトのテーマカラー（白 #ffffff / 深紅 #b92a4f / 黒 #000000）をベースに、ゲーム内UIを統一。
純粋な黒の代わりに **漆黒緑（#0a1a10）** を採用し、山間の村という舞台設定に合わせた奥行きのある暗さを表現する。
深紅と深緑は補色関係に近く、互いを引き立てる配色。

##### テーマカラー

| 役割 | 色名 | HEX | Godot Color |
|---|---|---|---|
| メイン | 白 | #ffffff | `Color(1.0, 1.0, 1.0)` |
| アクセント | 深紅 | #b92a4f | `Color(0.725, 0.165, 0.31)` |
| アクセント（明） | 薄紅 | #d94a6f | `Color(0.851, 0.29, 0.435)` |
| アクセント（暗） | 暗紅 | #8a1f3b | `Color(0.541, 0.122, 0.231)` |
| ベース（暗） | 漆黒緑 | #0a1a10 | `Color(0.039, 0.102, 0.063)` |

##### 用途別カラー

| 用途 | 色 | 値 |
|---|---|---|
| テキスト（主） | 白 | `Color(1.0, 1.0, 1.0, 1.0)` |
| テキスト（副） | グレー | `Color(0.7, 0.7, 0.7, 1.0)` |
| テキスト（ホバー） | 薄紅 | `Color(0.851, 0.29, 0.435, 1.0)` |
| テキスト（無効） | 暗灰 | `Color(0.4, 0.4, 0.4, 1.0)` |
| タイトルボタン | ダーク | `Color(0.224, 0.196, 0.2, 1.0)` |
| ゲーム画面オーバーレイ | 半透明漆黒緑 | `Color(0.039, 0.102, 0.063, 0.5)` |
| パネル背景 | 漆黒緑 | `Color(0.039, 0.102, 0.063, 0.8)` |
| 設定/メニュー背景 | 漆黒緑（明） | `Color(0.06, 0.12, 0.08, 0.9)` |
| ボーダー（ホバー） | 深紅 | `Color(0.725, 0.165, 0.31, 1.0)` |
| スキップ中 | 深紅 | `Color(0.725, 0.165, 0.31, 1.0)` |
| アウトライン | 黒 | `Color(0, 0, 0, 0.9)` |

##### カラーテーマ提案（検討中）

**現状の課題**: 漆黒緑 `#0a1a10` がゲームの雰囲気（感動70%、ミステリー25%、ホラー5%）に対してホラー色が強すぎる。
...
| --------------- | -------- | ------- | ---------------------------- | ------------------------ |
| メイン          | 白       | #ffffff | `Color(1.0, 1.0, 1.0)`       | テキスト、純粋           |
| アクセント      | 深紅     | #b92a4f | `Color(0.725, 0.165, 0.31)`  | 声、命、情熱             |
| ベース（暗）    | 深い青紫 | #1a1a2e | `Color(0.102, 0.102, 0.18)`  | 10月の夜空、記憶の深淵   |
| サブアクセント  | 薄青灰   | #8da4b8 | `Color(0.553, 0.643, 0.722)` | 10月の空、郷愁、懐かしさ |
| 霊体/ミステリー | 淡青白   | #c8d5e0 | `Color(0.784, 0.835, 0.878)` | 白い服の少年、霊体       |

**配色の意図**:

- 深紅 × 深い青紫: 情熱と静寂の対比（補色関係に近い）
- 薄青灰: プロローグの「10月にしては青すぎる景色」を表現
- 淡青白: 霊体やミステリアスな要素
...
```gdscript
# 深紅のバリエーション
COLOR_CRIMSON_LIGHT = Color(0.851, 0.29, 0.435)   # #d94a6f 薄紅（ホバー）
COLOR_CRIMSON_DARK  = Color(0.541, 0.122, 0.231)  # #8a1f3b 暗紅（押下）

# 深い青紫のバリエーション
COLOR_DARK_BLUE_LIGHT = Color(0.165, 0.165, 0.306)  # #2a2a4e やや明るい青紫
COLOR_DARK_BLUE_DARK  = Color(0.071, 0.071, 0.118)  # #12121e より暗い青紫
```
...
| -------------------- | --------------- | --------------------------------- |
| テキスト（主）       | 白              | `Color(1.0, 1.0, 1.0, 1.0)`       |
| テキスト（副）       | 薄青灰          | `Color(0.553, 0.643, 0.722, 1.0)` |
| テキスト（ホバー）   | 薄紅            | `Color(0.851, 0.29, 0.435, 1.0)`  |
| テキスト（無効）     | 暗灰            | `Color(0.4, 0.4, 0.4, 1.0)`       |
| 背景（暗）           | 深い青紫        | `Color(0.102, 0.102, 0.18, 1.0)`  |
| 背景（オーバーレイ） | 半透明青紫      | `Color(0.102, 0.102, 0.18, 0.9)`  |
| パネル背景           | 深い青紫（85%） | `Color(0.102, 0.102, 0.18, 0.85)` |
| ボーダー             | 薄青灰          | `Color(0.553, 0.643, 0.722, 1.0)` |
| ボーダー（ホバー）   | 深紅            | `Color(0.725, 0.165, 0.31, 1.0)`  |
| 霊体エフェクト       | 淡青白          | `Color(0.784, 0.835, 0.878, 1.0)` |
| スキップ中           | 深紅            | `Color(0.725, 0.165, 0.31, 1.0)`  |
| アウトライン         | 黒              | `Color(0, 0, 0, 0.9)`             |

**選定理由**:

- ゲームの本質は「感動」（70%）であり、ホラー要素は極小（5%、マルチシナリオのみ）
- プロローグの重要モチーフ「10月にしては青すぎる景色」を薄青灰で表現
- 深紅と深い青紫は補色関係に近く、互いを引き立てる
- 深い青紫は十分に暗いため、ホラー要素にも対応可能
- 視認性が高い（白文字 × 深い青紫 = 高コントラスト）
...
| 役割            | 色名         | HEX     | Godot Color                  | イメージ         |
| --------------- | ------------ | ------- | ---------------------------- | ---------------- |
| メイン          | 白（生成り） | #f5f5dc | `Color(0.961, 0.961, 0.863)` | 和紙、温かみ     |
| アクセント      | 深紅         | #b92a4f | `Color(0.725, 0.165, 0.31)`  | 声、命、情熱     |
| ベース（暗）    | 墨色         | #1c1c1c | `Color(0.11, 0.11, 0.11)`    | 墨、古い村       |
| サブアクセント  | 灰青         | #6b7c93 | `Color(0.42, 0.486, 0.576)`  | 秋の空、郷愁     |
| 霊体/ミステリー | 薄墨         | #b8c5d6 | `Color(0.722, 0.773, 0.839)` | 霊体、記憶の薄れ |
...
# 墨色のバリエーション
COLOR_INK_LIGHT = Color(0.18, 0.18, 0.18)  # #2d2d2d やや明るい墨色
COLOR_INK_DARK  = Color(0.05, 0.05, 0.05)  # #0d0d0d より暗い墨色
```
...
| 用途                 | 色          | 値                                |
| -------------------- | ----------- | --------------------------------- |
| テキスト（主）       | 生成り      | `Color(0.961, 0.961, 0.863, 1.0)` |
| テキスト（副）       | 灰青        | `Color(0.42, 0.486, 0.576, 1.0)`  |
| テキスト（ホバー）   | 薄紅        | `Color(0.851, 0.29, 0.435, 1.0)`  |
| テキスト（無効）     | 暗灰        | `Color(0.4, 0.4, 0.4, 1.0)`       |
| 背景（暗）           | 墨色        | `Color(0.11, 0.11, 0.11, 1.0)`    |
| 背景（オーバーレイ） | 半透明墨色  | `Color(0.11, 0.11, 0.11, 0.9)`    |
| パネル背景           | 墨色（85%） | `Color(0.11, 0.11, 0.11, 0.85)`   |
| ボーダー             | 灰青        | `Color(0.42, 0.486, 0.576, 1.0)`  |
| ボーダー（ホバー）   | 深紅        | `Color(0.725, 0.165, 0.31, 1.0)`  |
| 霊体エフェクト       | 薄墨        | `Color(0.722, 0.773, 0.839, 1.0)` |
| スキップ中           | 深紅        | `Color(0.725, 0.165, 0.31, 1.0)`  |
| アウトライン         | 黒          | `Color(0, 0, 0, 0.9)`             |

**選定理由**:

- 日本の田舎を舞台にした物語に合う和風の色調
- 墨色は深みがあり、記憶の闇を表現
- 生成りのテキストは和紙や古い手紙を想起させ、温かみがある
- 灰青が秋の情緒を演出
- シンプルで落ち着いた配色
...
- ベース（暗）: 深緑青 `#0f2027` - 海の深さ、記憶の闇
- サブアクセント: 青緑 `#4a7c8c` - 海、地蔵焚の立地

**提案D: モダン・感情重視**

- ベース（暗）: 深灰 `#2a2a2a` - 記憶の闇、無彩色
- サブアクセント: 青灰 `#7a8a99` - 冷静、思考
...
- **通常時**: グレー文字（キャプションサイズ）、ホバーで薄紅に変化
- **スキップ中**: 「スキップ」ボタンの文字色が深紅に変化
- **実装**: `scripts/ui/bottom_menu.gd`

#### ポーズメニュー

- **開閉**: Escapeキー（下部メニューの「メニュー」ボタンでも開く）
- **実装**: `scripts/ui/pause_menu.gd`（CanvasLayer layer=40 上に配置）
- **デザイン**: 足跡と統一した和風デザイン（装飾線タイトル ── メニュー ──、深紅アクセント、フラットボタン+左ボーダーホバー）
- **背景**: 漆黒緑 95%不透明 `Color(COLOR_BASE_DARK, 0.95)`
- **ボタン**: ゲームに戻る / 足跡 / セーブ / ロード / トロフィー / 設定 / タイトルへ戻る
- **状態**: 足跡は実装済み、セーブ・ロード・トロフィーは未実装（disabled）
- **閉じるヒント**: 下部に「Esc」表示
...
- **デザイン**: 和風ノベル風（装飾線タイトル ── 足跡 ──、深紅左ボーダーのカード型エントリ）
- **背景**: 漆黒緑 85%不透明 `Color(COLOR_BASE_DARK, 0.85)`
