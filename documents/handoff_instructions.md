...
├── days/                                      # 日付ごとのメインシナリオ
│   └── day_1010/
│       ├── main.json                          # 10月10日メイン（バス到着〜自由時間開始）
│       └── exploration.json                   # 10月10日 4方向探索の選択肢
├── branches/                                  # 探索ルートの分岐シナリオ
│   └── day_1010/
│       ├── day_1010_b_1.json                  # 北（海の入口）
│       ├── day_1010_b_2.json                  # 東（湧き水）
│       ├── day_1010_b_3.json                  # 南（お寺）
│       ├── day_1010_b_4.json                  # 西（バス停方面）
│       ├── day_1010_c_1.json                  # 北→北東（岩場）
│       ├── day_1010_d_1.json                  # 北→北東→南（バス停）
│       ├── day_1010_d_2.json                  # 北→北東→西（海水浴場へ）
│       ├── day_1010_d_3.json                  # 北→北東→北（未完成）
│       ├── day_1010_e_1.json                  # 東→北（バス停）
│       ├── day_1010_e_2.json                  # 東→南（神社）
│       └── day_1010_e_3.json                  # 東→西（未完成）
├── shared/                                    # 共用シナリオ（複数ルートから呼ばれる）
│   ├── shared_day_1010_c.json                 # 海水浴場（ep_2 + 霊体）
│   └── shared_ep_3_after.json                 # バス停の後（霊体 + イロ迎え）
└── episodes/                                  # 過去の記憶（回想シーン）
...
**選択肢ID → ファイル名の対応**:
- `opt_day_1010_b_1` → `branches/day_1010/day_1010_b_1.json`
- `opt_day_1010_c_1` → `branches/day_1010/day_1010_c_1.json`
- `opt_day_1010_c_2` → `shared/shared_day_1010_c.json` （共用シナリオ）

**命名規則の説明**:
- **b_N**: 1枠目の選択肢（北/東/南/西の4方向）
- **c_N**: 2枠目の選択肢（1枠目の各方向からさらに分岐）
- **d_N, e_N**: 3枠目以降の選択肢（さらなる分岐）
- **shared_**: 複数ルートから呼び出される共用シナリオ
