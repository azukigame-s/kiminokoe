...
- **BGM配置（一部）**: 完了（タイトル `悠久の彼方.mp3` 確定・実装済み、探索後フェーズ `忘却の都.mp3` 実装済み）
...
- **BGM配置（残り）**: 夕食・日常 / 夜・内省 / 回想 / 不穏 — 下記「BGM計画」を参照
...
| # | 種類 | 決定曲 | 雰囲気 | 使用箇所 | 状態 |
|---|------|--------|--------|---------|------|
| 1 | タイトル画面 | **悠久の彼方.mp3** | ノスタルジック、儚い、神秘的 | タイトル画面 | ✅ 実装済み |
| 2 | メインテーマ（日常・探索） | **忘却の都.mp3** | リュート、望郷 | バス到着〜イロ再会、探索フェーズ | ✅ 実装済み（探索後フェーズ） |
| 3 | 夕食・日常 | Old home / あさぎりのもり | アコギ、牧歌的、温かい | 19:00 夕食シーン | 未配置 |
| 4 | 夜・内省 | 秋の想い出 / 桜花の記憶 / 冬待人 | ピアノソロ、しっとり | 21:00 寝室、一人で過ごす夜 | 未配置 |
| 5 | 回想 | Ancient Travelers | ハープ、民族音楽風、切ない+温かい | ep_01〜ep_07のフラッシュバック | 未配置 |
| 6 | 不穏・焚き場 | 悲しい記憶（別作曲者） | ピアノ+効果音、不安、シンプル | 焚き場シーン、霊体描写 | 未配置 |
| 7 | トラウマ（ep_00） | — | 無音またはSEのみ | ep_00プロローグ（事故の記憶） | 未配置（無音で仮運用中） |
| 8 | 童歌（ふたこじぞう） | — | 子供の声や単純な旋律、不気味さ | shared_warabeuta（PoemDisplay） | 未配置 |

**備考:**
- タイトルは「悠久の彼方」に確定。「あさぎりのもり」は夕食・日常の候補に回す
- 夜・内省は3曲候補あり。聴き比べて1曲に絞るか場面ごとに使い分ける
- 焚き場の演出方針: BGMフェードアウト → 無音/環境音 → 核心で「悲しい記憶」を控えめに導入
