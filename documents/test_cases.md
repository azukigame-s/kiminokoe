...
#### トゥルーエンド（2ケース）
- [x] TC-101: 南→西→北（2周プレイ）
- [x] TC-102: 西→南→北（2周プレイ）

#### 高回収（2ケース）
- [x] TC-201: 北→東→南→西（複数周プレイ）
- [x] TC-202: 東→北→南（複数周プレイ）
...
5. [x] 21:00共通シーンに到達する
6. [x] 22:00分岐シーンに到達する
7. [x] エンディング分岐が正しく判定される
8. [x] トロフィーが正しく取得される
9. [x] 秘密基地フラグが正しく設定される（TC-008, TC-009, TC-011, TC-101, TC-102）
10. [x] 白い服の少年の描写が正しく表示される（ep_1, ep_2, ep_3取得時）
...
- [x] トロフィー10個獲得状態＋プレイ時間を満たしている状態で童歌シナリオをオートプレイすると、トロフィーが獲得できない
- [x] トロフィー10個獲得状態＋プレイ時間を満たしている状態で童歌シナリオを閲覧すると、トロフィーが獲得できる
...
- [x] ~~スプラッシュ画面でマウスクリックを行うと進行不能になってしまう~~ → splash_scene.gd: signal _advance_requested パターンで修正（VideoStreamPlayer.stop() がGodot4では finished を発火しない問題）
- [x] ~~体験版EDに表示される取得可能なトロフィーは12ではなく11ではないか？~~ → trophy_manager.gd: demo_episode_ids で ep_4 を除外、合計11個に修正
- [x] ~~バスのピンポンSEが削除されているので復活が必要~~ → main.json: バスアナウンス直前に `bus_button_sfx.mp3` の sfx コマンドを追加